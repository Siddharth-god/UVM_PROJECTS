...
/* -- Too big for it's own good fk it. 
...
class sb extends uvm_scoreboard;
    ...
    function new(string name, uvm_component parent);
        super.new(name,parent);
        fifo_mon_port = new("fifo_mon_port");
    endfunction 

    function exp_out(ref xtn xtn_h);
        if(!xtn_h.rstn)
            exp_q = 0;
        else if(xtn_h.t)
            exp_q = ~exp_q;
    ...
    function check_out(xtn xtn_h1); // this data we get from fifo -- from monitor
        xtn local_xtn; 
        if(!$cast(local_xtn, xtn_h1.clone())) // cloning the mon data into local 
            `uvm_fatal(get_full_name(),"cast failed in scoreboard.")


            if(exp_out(local_xtn)) // passing local to check exp out - this is mon data
                begin 
                    if(xtn_h1.compare(local_xtn)) // in exp out we have "ref" so whatever changed inside reflected on local now check exp out with mon data. 
                        `uvm_info(get_type_name(), $sformatf("Scoreboard - Data Match successful"), UVM_MEDIUM)
                    
                    else 
                        `uvm_error(get_type_name(), $sformatf(
                            "\n Scoreboard Error [Data Mismatch]: \n Received Transaction:\n %s \n Expected Transaction: \n %s",
                            xtn_h1.sprint(), local_xtn.sprint()))

                end
            else
                uvm_report_info(get_type_name(), $psprintf("No Data written \n %s", xtn_h1.sprint()));
