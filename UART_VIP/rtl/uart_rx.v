...
*********************************************************************************************/
 ...
 // Works in conjuction with the receiver state machine
   ...
   always @(LCR)
     case(LCR[3:0])
       4'b0000 : toc_value = 10'd447; // 7 bits ==> 1 bit needs 16 baud pulses, so 7 bits = 16x7 => for 1 frame ==> If we need for 4 frames then 7x16x4 = 448 ==> we start from 0 so 447
   ...
   always @(posedge PCLK) 
     begin
       if(PRESETn==1'b0)
         counter_b <= 8'd159; // 1 frame with 10 bits needs 160 baud ticks 
       else if (RXD)
